...
const BadgeContent = forwardRef<HTMLDivElement, { score: number; recommendation: string; className?: string }>(
  ({ score, recommendation, className }, ref) => {
    const getScoreColor = () => {
    ...
    return (
      <div 
        ref={ref}
        className={cn(
          "inline-flex items-center gap-1.5 px-2 py-1 rounded-full border text-xs font-medium cursor-help",
          getScoreColor(),
          className
        )}
      >
...
export function AIFitBadge({ score, recommendation, justification, className }: AIFitBadgeProps) {
  if (!justification) {
    return <BadgeContent score={score} recommendation={recommendation} className={className} />;
  }
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <BadgeContent score={score} recommendation={recommendation} className={className} />
        </TooltipTrigger>
